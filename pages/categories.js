  ...
  useEffect(() => {
    const LoadContracts = async () => {
      ...
      if (nftMarketWorkObject && nftMarketWorkObject) {
        ...
        const deployedTokenContract = await new web3Api.web3.eth.Contract(tokenAbi,tokenAddress);
          ...
          data.map(async (item) => {
            ...
            const priceToWei = Web3.utils.fromWei(
              ...
              "ether"
                ...
                const replaceIPFsName =  nftUrl.toString().replace("ipfs.infura.io","mrbebo.infura-ipfs.io")
            ...
            const replaceIPFsImageName =  oldImageUrl.toString().replace("ipfs.infura.io","mrbebo.infura-ipfs.io")
    ...
    <>
      ...
      <div className="bg-[#202225] dark:bg-white">
        <div className="w-full 2xl:max-w-screen-2xl h-auto pt-[104px] m-auto">
          <div className="flex flex-col mx-8 sm:mx-16 lg:mx-[3vw] space-y-6 py-12">
            {/* custom breadcrubs */}
            ...
            {/* <div className="flex flex-col md:flex-row space-y-2 md:space-y-0">
              ...
              {btnCategories.map((item, index) => (
                                <button key={"btn-category" + index.toString()} className={classNames(index === current ? 'bg-[#FFCC33] dark:bg-gray-800 text-white' : 'border border-[#2C3166] dark:border-gray-400 bg-[#1E1E1E] dark:bg-white text-[#919CC1] dark:text-gray-800', 'text-xs rounded-full px-4 py-1.5')} onClick={() => {setCurrent(index)
                                    setCategory(item)
                                }}>{item}</button>
                            ))}
              </div>
